...
- 按项目聚合排行
- 数据源分布
- 3D Lab —— Token Landscape：最近一年的每日消耗排成等轴测日历地形（x 轴为周、z 轴为星期、一天一根实心柱）。柱高与颜色取自当天用量的**分位等级**（0–4 级，阈值为活跃日的 P50 / P75 / P90），所以地形是五级台地而不是几根孤零零的尖刺；固定看一年、不跟随上方的时间范围（该页也因此隐藏了范围选择器）；可拖拽旋转、滚轮缩放、悬浮查看某天明细。左侧给出年度总量、峰值日、活跃率与最长连续天数。
...
- 3D Lab 只画有数据的那几周（前后空白周会被裁掉），所以"最近一年"是数据窗口而不是画面宽度。
